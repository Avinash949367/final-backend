...
exports.createSlot = async (req, res) => {
    try {
        ...
        while (retryCount < maxRetries) {
            const slotCount = await Slot.countDocuments({ stationId: station._id });
...
exports.createBooking = async (req, res) => {
    try {
        ...
        const { slotId, bookingStartTime, durationHours, vehicleId, vehicleNumber, paymentMethod, amountPaid } = req.body;
        ...
        if (!slotId || !bookingStartTime || !durationHours || !paymentMethod) {
        ...
        // Find vehicle by vehicleId if provided (for mobile app compatibility)
        let vehicle = null;
        if (vehicleId) {
...
exports.getSlotBookingsByUserId = async (req, res) => {
    try {
        ...
        const bookings = await SlotBooking.find({ userId: new mongoose.Types.ObjectId(userId) }).populate('slotId', 'slotId').populate('vehicleId', 'number').populate('stationId', 'name').sort({ bookingStartTime: -1 });
        res.status(200).json(bookings);
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
